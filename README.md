...
Two, both stored encrypted via the OS keychain, both validated against the real
service rather than pattern-matched:

- `searchApiKey` — SerpApi. Required. Validated against their account endpoint,
  which also returns the remaining-search count shown in the header.
- `anthropicApiKey` — optional. Validated with a 4-token request. Unlocks a
  **Rewrite** button on each generated email. Roughly a tenth of a cent per
  rewrite on Haiku. The app is fully usable without it, because personalisation
  comes from the findings rather than from generated prose.

The rewrite prompt forbids inventing findings, prices or credentials, and
forbids naming a specific technical problem the template did not name — so a
rewrite cannot turn a vague, safe email into a specific claim about someone's
website.
