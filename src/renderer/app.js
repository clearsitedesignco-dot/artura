...
/* The Start here list tracks what has actually happened, so it stops being
   three frozen rows the moment a member does something. */
function renderStartHere(){
  const box = $('startHere'); if (!box) return;
  const L = STATE.leads;
  const steps = [
    { t:'Add your SerpApi key', d:'Free to start, no card needed.',
      done: hasKey, go:'keys', icon:'i-keys', cta:'Add key' },
    { t:'Run your first search', d:'Pick a business type and a zip code.',
      done: L.length > 0, go:'scraper', icon:'i-scraper', name:'Lead Scraper', cta:'Open scraper',
      note: L.length ? `${L.length} leads collected` : null },
    { t:'Send some to the Call Organizer', d:'Tick the ones worth calling.',
      done: L.some(x => x.in_organizer), go:'organizer', icon:'i-calls', name:'Call Organizer', cta:'Open organizer',
      note: L.filter(x => x.in_organizer).length ? `${L.filter(x=>x.in_organizer).length} in the queue` : null },
    { t:'Make your first calls', d:'Work the queue and log what happened.',
      done: L.some(x => x.attempts > 0), go:'organizer', icon:'i-calls', name:'Call Organizer', cta:'Start calling',
      note: L.filter(x => x.attempts>0).length ? `${L.filter(x=>x.attempts>0).length} called` : null },
    { t:'Book a meeting', d:'The only goal of a cold call.',
      done: L.some(x => x.meeting_at), go:'meetings', icon:'i-meet', name:'Meetings', cta:'See meetings',
      note: L.filter(x=>x.meeting_at).length ? `${L.filter(x=>x.meeting_at).length} booked` : null }
  ];
  ...
  box.innerHTML = steps.map((st, i) => `
    <div class="row">
      ...
      <div><div class="rt" ${st.done?'style="color:var(--ink-3);text-decoration:line-through"':''}>${st.t}</div>
        <div class="rd">${st.note || st.d}</div></div>
      <div class="rr">
        ${st.done
          ? '<span class="status" style="color:var(--ink-3)"><span class="pip live"></span>Done</span>'
          : i === firstOpen
            ? `<button class="btn pri" data-go="${st.go}" data-icon="${st.icon}" ${st.name?`data-name="${st.name}"`:''}>${st.cta}</button>`
            : '<span class="status" style="color:var(--ink-3)">Next up</span>'}
      </div>
    </div>`).join('');
...
/* ---- Start here: a live checklist, not static rows ---- */
function renderStartHere(){
  const box=$('startHere'); if(!box) return;
  const L=db.all();
  const steps=[
    { t:'Add your search key',        d:'Free to start, and no credit card needed.',
      done:hasKey, go:'keys', icon:'i-keys', cta:'Add key' },
    { t:'Fill in your details',       d:'Used in every email the app writes for you.',
      done:profileReady(), go:'profile', icon:'i-about', cta:'Fill in' },
    { t:'Run your first search',      d:'Pick a business type and a zip code.',
      done:L.length>0, go:'scraper', icon:'i-scraper', name:'Lead Scraper', cta:'Open scraper',
      needs:hasKey },
    { t:'Send some leads to the queue', d:'Tick the ones worth calling in the results table.',
      done:db.organizer().length>0, go:'scraper', icon:'i-scraper', name:'Lead Scraper', cta:'Pick leads',
      needs:L.length>0 },
    { t:'Make your first calls',      d:'Work the queue and book meetings.',
      done:L.some(x=>x.attempts>0), go:'organizer', icon:'i-calls', name:'Call Organizer', cta:'Open queue',
      needs:db.organizer().length>0 },
    { t:'Hold your first meeting',    d:'Screen share, show what you found, give the price.',
      done:L.some(x=>x.meeting_outcome), go:'meetings', icon:'i-meet', name:'Meetings', cta:'Open meetings',
      needs:L.some(x=>x.meeting_at) }
  ];
  const firstOpen = steps.findIndex(s=>!s.done);
  box.innerHTML = steps.map((s,i)=>`
    <div class="row${s.done?' rdone':''}">
      <span class="rn">${String(i+1).padStart(2,'0')}</span>
      <div><div class="rt">${s.t}</div><div class="rd">${s.d}</div></div>
      <div class="rr">
        ${s.done
          ? '<span class="status" style="color:var(--ink-3)"><span class="pip live"></span>Done</span>'
          : (s.needs===false
              ? '<span class="status" style="color:var(--ink-3)">Locked</span>'
              : `<button class="btn ${i===firstOpen?'pri':''}" data-go="${s.go}" data-icon="${s.icon}"${s.name?` data-name="${s.name}"`:''}>${s.cta}</button>`)}
      </div>
    </div>`).join('');
  box.querySelectorAll('[data-go]').forEach(b=>b.addEventListener('click',()=>go(b)));
}
...
function renderMeetings(){
  ...
  $('mt-list').querySelectorAll('.mcard').forEach(card=>{
    ...
    card.querySelectorAll('.ob[data-out]').forEach(b=>b.addEventListener('click', async () => {
      ...
      if(o==='won'){
        const r=await askUser({ title:`You won ${L.name}`, confirm:'Log the win',
          fields:[{label:'What did the deal come to?', type:'number', min:0, step:'50',
                   value:L.deal_value||'', placeholder:'800'}] });
        if(!r) return;
        const v=Number(r[0])||0;
        await db.update(L.place_id,{ meeting_outcome:'won', deal_value:v });
        await db.logEvent('won',`Won <b>${L.name}</b> at $${v.toLocaleString()}`);
      } else if(o==='lost'){
        ...
        const r=await askUser({ title:`Move the meeting with ${L.name}`, confirm:'Move it',
          fields:[{label:'New date', type:'date', value:(L.meeting_at||'').slice(0,10)},
                  {label:'New time', type:'time', value:(L.meeting_at||'').slice(11,16)||'10:00'}] });
        if(!r || !r[0]) return;
        await db.update(L.place_id,{ meeting_at:r[0]+'T'+(r[1]||'10:00'), confirmed:0 });
      }
...
let outOnlyUnreached=true;

/* Who outreach is for. By default: people the phone did not reach.
   Someone who answered and said no should not then get an email. */
function outTargets(leads){
  if(!outOnlyUnreached) return leads;
  const reached=['booked','no','maybe','blacklist'];
  return leads.filter(l=>!reached.includes(l.call_status));
}
...
function outreachPool(){
  // Email is the follow-up for people the phone did not reach. Anyone already
  // spoken to belongs in the Call Organizer, not in a mail merge.
  const L=db.all();
  const unreached = L.filter(l =>
    !['booked','no','blacklist','maybe'].includes(l.call_status) &&   // already dealt with by phone
    !l.emailed_at                                                     // not already emailed
  );
  return { all:L, pool:unreached };
}

function renderOutreach(){
  const { all:leads, pool } = outreachPool();
  ...
  if(outStage===1){
    const enriched=target.filter(l=>l.enriched).length;
    ...
    box.innerHTML=`<div class="tmpl">
      ...
      <label class="cb" style="margin-bottom:14px"><input type="checkbox" id="onlyUnreached" ${outOnlyUnreached?'checked':''}>
        Only the ones the phone didn't reach</label>
      <div class="h2" style="font-size:11.5px;color:var(--ink-3);margin:-8px 0 16px;line-height:1.55;max-width:560px">
        A call beats an email every time, so email is the follow-up for people who didn't pick up.
        With this on, anyone you've already spoken to is left alone — no one gets both.</div>
      <button class="btn pri lg" id="runEnrich">${enriched?'Run again':`Find emails for ${outTargets(leads).length} leads`}</button>
    ...
    const ou=$('onlyUnreached');
    if(ou) ou.addEventListener('change',()=>{ outOnlyUnreached=ou.checked; renderOutreach(); });
    $('runEnrich').addEventListener('click',async()=>{
      ...
      await enrichBatch(outTargets(leads),(d,t,f)=>{ $('enrProg').innerHTML=`<div class="scanline"><span class="spin"></span> ${d} of ${t} · ${f} emails found <span class="prog"><i style="width:${Math.round(d/t*100)}%"></i></span></div>`; });
      renderOutreach();
    });
  ...
  if(outStage===2){
    const checked=target.filter(l=>l.checked).length;
    box.innerHTML=`<div class="tmpl">
      ...
      <button class="btn pri lg" id="runCheckAll">${checked?'Run again':`Check ${outTargets(leads).filter(l=>l.website).length} sites`}</button>
    ...
    $('runCheckAll').addEventListener('click',async()=>{
      ...
      await checkBatch(outTargets(leads),(d,t)=>{ $('chkProg').innerHTML=`<div class="scanline"><span class="spin"></span> ${d} of ${t} <span class="prog"><i style="width:${Math.round(d/t*100)}%"></i></span></div>`; });
      renderOutreach();
    });
  ...
  if(outStage===3){
    let targets=outTargets(leads).filter(l=>l.email);
    if(!targets.length){
    ...
    box.innerHTML=`
      <div class="h-sec" style="margin-top:0">${targets.length} ${targets.length===1?'email':'emails'} ready</div>
      <div id="mailList"></div>

      <details class="tmplbox">
        <summary>Edit the wording of these emails</summary>
        <div class="tmpl" style="margin-top:14px">
          <div class="tlab">Businesses that have a website</div>
          ...
          <div class="h2" style="margin-top:12px;font-size:11.5px;color:var(--ink-3);line-height:1.55">
            Notice what the template does not do: it never names the problems. Saying you found
            something specific and offering to show them is what gets the meeting — and spelling out
            a stranger's security holes in writing reads badly, however well you mean it.</div>
        </div>
        <div class="tmpl">
          <div class="tlab">Businesses with no website at all</div>
    ...
    const redraw=()=>{
        ...
        return `<div class="mail ${L.emailed?'sent':''}" data-i="${i}">
          ...
          <div class="mb"><div class="subj">Subject: ${m.subject}</div>
            ...
            <div class="mact"><button class="btn" data-copy="${i}">Copy email</button>
              ${hasAI?`<button class="btn" data-polish="${i}">Rewrite</button>`:''}
              <button class="btn pri" data-sent="${i}">${L.emailed?'Sent':'Mark as sent'}</button></div>
            <div class="polishnote" data-pn="${i}"></div></div>
        </div>`;
      }).join('');
      ...
      $('mailList').querySelectorAll('[data-polish]').forEach(b=>b.addEventListener('click', async () => {
        const i=b.dataset.polish;
        const ta=$('mailList').querySelector(`[data-body="${i}"]`);
        const note=$('mailList').querySelector(`[data-pn="${i}"]`);
        const original=ta.value;
        b.disabled=true; b.innerHTML='<span class="spin"></span> Rewriting…';
        note.textContent='';
        try{
          const r=await unwrap(API.ai.polish({ body:original, business:targets[+i].name }));
          ta.value=r.body;
          note.innerHTML='Rewritten. <span class="lnk" data-undo="'+i+'">Put the original back</span>';
          note.querySelector('[data-undo]').addEventListener('click',()=>{
            ta.value=original; note.textContent='Original restored.';
          });
        }catch(err){
          note.textContent =
            err.code==='BAD_KEY' ? 'Your Anthropic key was rejected. Check it on the API Keys screen.'
          : err.code==='QUOTA'   ? 'That Anthropic account is out of credit. The email below is unchanged.'
          : err.code==='NETWORK' ? 'Could not reach Anthropic. The email below is unchanged.'
          : 'The rewrite did not come back. The email below is unchanged.';
        }finally{ b.disabled=false; b.textContent='Rewrite'; }
      }));
      ...
      $('mailList').querySelectorAll('[data-sent]').forEach(b=>b.addEventListener('click', async () => {
        ...
        targets=outreachPool().pool.filter(l=>l.email);
...
let projects=[], pjOpen=null, phaseOpen=0, pjMode='steps';   // 'steps' | 'all'
...
function nextTask(pj){
  for(let i=0;i<PHASES.length;i++)
    for(let j=0;j<PHASES[i].tasks.length;j++)
      if(!taskDone(pj,i,j)) return {i,j};
  return null;
}

function renderProjectSteps(){
  const pj=projects[pjOpen], pr=projProgress(pj), at=nextTask(pj);
  ...
  const header=`
    <div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap;margin-bottom:24px">
      ...
      <div><div style="font-family:var(--display);font-size:21px;font-weight:600;letter-spacing:-.02em">${pj.name}</div>
        <div style="font-size:12.5px;color:var(--ink-3);margin-top:3px">${pr.done} of ${pr.total} steps done${(()=>{
          const ci=currentPhase(pj), t=PHASES[ci].tasks.find((_,k)=>!taskDone(pj,ci,k));
          return t? ' &middot; next: '+t.t : ' &middot; finished';
        })()}</div></div>
      <div style="margin-left:auto;display:flex;align-items:center;gap:14px">
        <div class="pbar" style="width:200px"><i style="width:${Math.round(pr.done/pr.total*100)}%"></i></div>
        <button class="btn" id="pjToggle">See every step</button>
      </div>
    </div>`;

  if(!at){
    $('pj-detail').innerHTML=header+`<div class="empty">
      <span class="bigico"><svg viewBox="0 0 24 24"><use href="#i-launch"/></svg></span>
      <h3>${pj.name} is finished</h3>
      <p>Every step done, from the first call through to handover. Ask for the testimonial while they are still delighted.</p></div>`;
    wireProjectHeader(); return;
  }

  const ph=PHASES[at.i], t=ph.tasks[at.j];
  const L=pj.place_id? db.byId(pj.place_id):null;
  const doneInPhase=ph.tasks.filter((_,j)=>taskDone(pj,at.i,j)).length;

  $('pj-detail').innerHTML=header+`
    <div class="stepcard">
      <div class="sc-head">
        <div class="sc-ph">Phase ${ph.n} · ${ph.t} · ${doneInPhase} of ${ph.tasks.length} done</div>
        <div class="sc-t">${t.t}${t.tool?` <span class="toolbadge">${t.tool}</span>`:''}</div>
        <div class="sc-d">${t.d}</div>
      </div>
      <div class="sc-body">
        ${t.why?`<div class="sc-why"><div class="bh">Why this matters</div><p>${t.why}</p></div>`:''}
        ${t.p?`<div class="sc-p"><div class="bh">Prompt to paste into Claude</div>
               <pre class="snip">${t.p.replace(/</g,'&lt;')}</pre>
               <button class="btn" id="pjCopy">Copy this prompt</button></div>`:''}
      </div>
      <div class="sc-foot">
        <button class="btn pri lg" id="pjDone">Done — next step</button>
        <button class="btn" id="pjSkip">Skip this one</button>
        ${t.tool?`<button class="btn ghost" id="pjOpen">Open ${t.tool}</button>`:''}
      </div>
    </div>
    <div class="upnext">
      <div class="bh">Coming up</div>
      ${ph.tasks.slice(at.j+1,at.j+4).map(x=>`<div class="un">${x.t}</div>`).join('') || '<div class="un">End of this phase</div>'}
    </div>`;

  wireProjectHeader();
  const mark=async v=>{ pj.done[at.i+'-'+at.j]=v; if(pj.id) await API.projects.done(pj.id,pj.done); renderProject(); };
  $('pjDone').addEventListener('click',()=>mark(true));
  $('pjSkip').addEventListener('click',()=>mark(true));
  const cp=$('pjCopy');
  if(cp) cp.addEventListener('click',()=>{
    navigator.clipboard?.writeText(t.p);
    cp.textContent='Copied'; setTimeout(()=>{ if(cp.isConnected) cp.textContent='Copy this prompt'; },1500);
  });
  const op=$('pjOpen');
  if(op) op.addEventListener('click',()=>{
    const map={'Lead Scraper':'scraper','Site Check':'sitecheck','Outreach':'outreach',
               'Call Organizer':'organizer','Meetings':'meetings','Prompt Library':'prompts'};
    const b=document.querySelector(`.item[data-go="${map[t.tool]}"]`); if(b) b.click();
  });
}
...
function wireProjectHeader(){
  ...
  const tg=$('pjToggle');
  if(tg) tg.addEventListener('click',()=>{ pjMode = pjMode==='steps'?'all':'steps'; renderProject(); });
}

function renderProject(){
  if(pjMode==='steps') return renderProjectSteps();
  ...
  $('pj-detail').innerHTML=`
    <div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap;margin-bottom:24px">
      ...
      <div style="margin-left:auto;display:flex;align-items:center;gap:14px">
        <div class="pbar" style="width:200px"><i style="width:${Math.round(pr.done/pr.total*100)}%"></i></div>
        <button class="btn" id="pjToggle">One step at a time</button>
      </div>
    </div>
    ${PHASES.map((ph,i)=>{
      const dn=ph.tasks.filter((_,j)=>taskDone(pj,i,j)).length;
      const isNow = i===currentPhase(pj);
      return `<div class="phase${isNow?' pnow':''}${dn===ph.tasks.length?' pdone':''}">
        <div class="ph" data-ph="${i}">
          <span class="pnum">${dn===ph.tasks.length?'&#10003;':ph.n}</span>
          <div><div class="pt2">${ph.t}${isNow?' <span class="nowtag">You are here</span>':''}</div>
               <div class="pd">${ph.d}</div></div>
        ...
        <div class="pbody ${i===phaseOpen?'open':''}">
          ${ph.tasks.map((t,j)=>{
            ...
            const isNext = !dn2 && ph.tasks.findIndex((_,k)=>!taskDone(pj,i,k))===j;
            return `<div class="task ${dn2?'done':''}${isNext?' tnext':''}">
            <input type="checkbox" class="rowcb" data-k="${i}-${j}" ${dn2?'checked':''} ${isAuto?'disabled':''}>
            <div style="flex:1">
              <div class="tt">${isNext?'<span class="nextdot"></span>':''}${t.t}${t.tool?` <span class="toolbadge">${t.tool}</span>`:''}</div>
...
function refreshCounts(){
  ...
  const wonLeads = db.all().filter(l=>l.meeting_outcome==='won');
  const closed = wonLeads.reduce((n,l)=>n+(Number(l.deal_value)||0),0);
  setNum($('closedTotal'), closed, '$');
  const cb=$('closedBreak');
  if(cb) cb.innerHTML = wonLeads.length
    ? `<b>${wonLeads.length}</b> ${wonLeads.length===1?'deal':'deals'} won<br>` +
      `average <b>$${Math.round(closed/wonLeads.length).toLocaleString()}</b>`
    : 'Nothing logged yet';
  renderStartHere();
  renderActivity();
...
/* ---- Anthropic key: optional, and only unlocks the Rewrite button ---- */
let hasAI = false;

function markAIConnected(model){
  hasAI = true;
  const p2=$('p2'), s2=$('s2'), b2=$('b2'), m2=$('m2');
  if(p2) p2.className='pip live';
  if(s2) s2.textContent='Connected';
  if(b2){ b2.textContent='Replace'; b2.classList.remove('pri'); }
  if(m2) m2.innerHTML=`<span>Working. Emails in Outreach now have a <b>Rewrite</b> button.${model?' Using '+model+'.':''}</span>`;
}
...
function wireAIKey(){
  ...
  b2.addEventListener('click', async () => {
    const v=(k2.value||'').trim();
    if(!v){ k2.focus(); return; }
    b2.disabled=true; b2.innerHTML='<span class="spin"></span> Checking…';
    s2.textContent='Checking'; p2.className='pip off';
    try{
      const res=await unwrap(API.keys.test('anthropicApiKey', v));
      markAIConnected(res.model);
      k2.value='';
    }catch(err){
      p2.className='pip bad'; s2.textContent='Rejected'; b2.textContent='Check key';
      m2.innerHTML =
        err.code==='NETWORK' ? '<span>Could not reach Anthropic. Check your internet connection — the key is probably fine.</span>'
      : err.code==='QUOTA'   ? '<span>That key works but the account has no credit on it. Add credit at console.anthropic.com, then check again.</span>'
      : err.code==='HOURLY'  ? '<span>Too many requests just now. Wait a minute and try again.</span>'
      : '<span>Anthropic did not accept that key. It should start with <b>sk-ant-</b> — check you copied all of it.</span>';
    }finally{
      b2.disabled=false;
      if(!hasAI) b2.textContent='Check key';
    }
  });
  if(k2) k2.addEventListener('keydown', e => { if(e.key==='Enter') b2.click(); });
}
...
/* =========================================================================
   Terms of Use. Explicit acceptance: a box they tick and a button they press,
   recorded with the version and date. "You agreed by downloading" is routinely
   unenforceable because nobody was shown anything.
   Keep TERMS_VERSION in step with legal/TERMS-DRAFT.md — bumping it makes
   every member accept again, which is what you want after a real change.
   ========================================================================= */
const TERMS_VERSION = '0.1';

const TERMS_HTML = `
<h4>1 · This is a tool, not a promise of income</h4>
<p>ArturaLabs helps you find businesses and organise your own selling. Whether
you earn anything depends on your effort, your skill, and your market.
<b>We do not promise you will make money, and most people who buy tools like
this earn nothing.</b> Any figure you have seen anywhere describes one person at
one moment. It is not typical and it is not a prediction about you.</p>

<h4>2 · You are running your own business</h4>
<p>You are independent. You are not our employee, partner, agent or
representative, and you must not present yourself as one. You are responsible
for your own registration, taxes, insurance, client agreements, and the work you
deliver. Nothing you say to a client is said on our behalf.</p>

<h4>3 · Your outreach is your responsibility</h4>
<p>This software helps you organise calls and emails. <b>It does not make them
legal.</b> The rules vary by state and country, and they apply to you. You agree
to:</p>
<ul>
  <li>Check Do Not Call registries where required, and honour them</li>
  <li>Stop contacting anyone who asks you to stop — permanently, on every channel</li>
  <li>Follow commercial email rules: honest headers, a real address, a working unsubscribe</li>
  <li>Never misrepresent who you are or why you are calling</li>
</ul>
<p>If you are fined or sued over your outreach, that is yours, not ours.</p>

<h4>4 · Site Check can be wrong</h4>
<p>It reads public information and reports what it finds. It can report a problem
that is not there and miss one that is. <b>Check anything yourself before you
tell a business owner about their website.</b> You are responsible for every
statement you make to a prospect.</p>
<p>You also agree not to use its findings to frighten, pressure or extort anyone.
Presenting them as a threat is a misuse of this software and may be a crime.</p>

<h4>5 · You pay third parties directly</h4>
<p>Search and AI providers are not ours. You use your own accounts and your own
keys, and <b>you pay them directly — we never charge you for their usage and
cannot refund it.</b> The AI rewrite feature is optional; if you use it, every
request is billed to your own Anthropic account, and you are responsible for
reading what it produced before you send it. Their terms apply to you as well as ours. If they change
their pricing or stop working, that is outside our control.</p>
<p>Keep your keys secret. They are encrypted on your computer. If you expose
one, the resulting charges are yours.</p>

<h4>6 · Your data lives on your computer</h4>
<p>Your leads, notes and keys are stored on this machine. We never receive them,
cannot see them, and <b>cannot recover them for you.</b> Backing them up is your
responsibility. You are also responsible for handling other people's information
lawfully.</p>

<h4>7 · Licence</h4>
<p>Your access is personal and non-transferable. Do not share, resell or
redistribute the software, your access, or the course material. We may end your
access if you break these terms.</p>

<h4>8 · No warranty, and limited liability</h4>
<p>The software is provided as is, with no warranty that it will be accurate,
uninterrupted or fit for any particular purpose. To the fullest extent the law
allows, we are not liable for lost profits, lost data, or any indirect loss, and
our total liability is limited to what you paid us in the previous twelve months.
You agree to cover our costs for any claim arising from your outreach, your
client work, or your breach of these terms.</p>

<h4>9 · Changes</h4>
<p>We may update these terms. Continued use after an update means you accept the
new version.</p>

<p style="margin-top:16px;color:var(--ink-3)">Version ${TERMS_VERSION}. The full
terms are available from the About screen at any time.</p>
`;

function showTermsModal(){
  const w = document.createElement('div');
  w.className = 'terms-modal';
  w.innerHTML = `<div class="tm">
    <div class="tmh">ArturaLabs — Terms of Use</div>
    <div class="tmb">${TERMS_HTML}</div>
    <div class="tmf"><button class="btn pri" data-close>Close</button></div></div>`;
...
function wireTerms(){
  const box = $('termsBox');
  if (box) box.innerHTML = TERMS_HTML;

  const tick = $('termsTick'), accept = $('termsAccept');
  if (tick) tick.addEventListener('change', () => { accept.disabled = !tick.checked; });

  if (accept) accept.addEventListener('click', async () => {
    me._termsVersion = TERMS_VERSION;
    me._termsAcceptedAt = new Date().toISOString();
    await unwrap(API.profile.set(me));
    onbGo(1);
  });

  const decline = $('termsDecline');
  if (decline) decline.addEventListener('click', () => {
    const w = document.createElement('div');
    w.className = 'terms-modal';
    w.innerHTML = `<div class="tm"><div class="tmh">You need to accept the terms to use ArturaLabs</div>
      <div class="tmb"><p>That is a fair decision, and there is no hard feelings in it — but the app cannot be used without agreeing to them.</p>
      <p>If something specific in there is the problem, get in touch and tell us which part. That is more useful to us than you walking away quietly.</p></div>
      <div class="tmf"><button class="btn" data-back>Back to the terms</button></div></div>`;
    document.body.appendChild(w);
    w.querySelector('[data-back]').onclick = () => w.remove();
  });

  const view = $('viewTerms');
  if (view) view.addEventListener('click', showTermsModal);
}
...
function wireOnboarding(){
  ...
  if (chk) chk.addEventListener('click', async () => {
    ...
    try {
      ...
      $('sc-locked').hidden = true; $('sc-main').hidden = false;
      if (searchesLeft != null) $('meterN').textContent = searchesLeft;
    } catch (err) {
      msg.className = 'onb-msg bad';
  ...
  const next = $('onbNext2');
  if (next) next.addEventListener('click', () => onbGo(3));

  const done = $('onbDone');
  if (done) done.addEventListener('click', finishOnboarding);
}

/* Electron does not implement window.prompt, so anything relying on it
   silently does nothing. This is a small in-app replacement. */
function askUser({ title, fields, confirm = 'Save' }) {
  return new Promise(resolve => {
    const wrap = document.createElement('div');
    wrap.className = 'ask-back';
    wrap.innerHTML = `
      <div class="ask">
        <div class="ask-t">${title}</div>
        <div class="ask-f">
          ${fields.map((f, i) => `
            <label>${f.label}
        ...
        <div class="ask-a">
          ...
          <button class="btn pri" data-ok>${confirm}</button>
        </div>
      </div>`;
    ...
    wrap.querySelector('[data-ok]').onclick = () => close(inputs.map(i => i.value.trim()));
    wrap.onclick = e => { if (e.target === wrap) close(null); };
    wrap.onkeydown = e => {
      if (e.key === 'Escape') close(null);
      if (e.key === 'Enter') close(inputs.map(i => i.value.trim()));
    };
  });
}
...
(async function boot(){
  try{
    ...
    wireAIKey();
    try{ if(await unwrap(API.keys.has('anthropicApiKey'))) markAIConnected(); }catch{}
    $('profpip').className='pip '+(profileReady()?'live':'off');
    document.querySelectorAll('.mock-only').forEach(el => el.hidden = PROVIDER !== 'mock');
    wireTerms();
    wireOnboarding();
    const agreed = prof && prof._termsVersion === TERMS_VERSION;
    if (!agreed) {
      $('onb').hidden = false; onbGo(0);
    } else if (!prof._onboarded) {
      $('onb').hidden = false; onbGo(hasKey ? 3 : 1);
    }
    const tw = $('termsWhen');
    if (tw && prof && prof._termsAcceptedAt) {
      tw.textContent = `Terms version ${prof._termsVersion} accepted on ` +
        new Date(prof._termsAcceptedAt).toLocaleDateString();
    }
    await syncState();
